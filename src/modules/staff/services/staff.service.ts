...
export class StaffService {
  async getStaff(query: StaffQueryDto, requestingUserRole: string) {
    ...
    const pendingCount = requestingUserRole === 'ADMIN' 
      ? await prisma.staff.count({ where: { isApproved: false } })
      : 0;
  ...
  async createStaff(staffData: StaffDto, requestingUserRole: string) {
    ...
    const staff = await prisma.staff.create({
      data: {
        ...staffData,
        role: staffData.role as any, // Cast to StaffRole enum
  ...
  async updateStaff(
    ...
    if (requestingUserRole !== "ADMIN" && id === requestingUserId) {
      ...
      allowedFields.forEach((field) => {
        if (staffData[field as keyof StaffUpdateDto] !== undefined) {
          filteredData[field] = staffData[field as keyof StaffUpdateDto];
        }
      });
    ...
    const staff = await prisma.staff.update({
      ...
      data: {
        ...staffData,
        role: staffData.role ? (staffData.role as any) : undefined, // Cast role to StaffRole enum if present
      },
      select: {
        id: true,
        firstName: true,
  ...
  async deactivateStaff(id: string, requestingUserRole: string) {

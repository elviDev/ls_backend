...
class EmailService {
    ...
    async sendVerificationEmail(email, token, name) {
        ...
        try {
            ...
            logger_1.default.error('Failed to send verification email', { email, error: error.message });
    ...
    async sendPasswordResetEmail(email, token, name) {
        ...
        try {
            ...
            logger_1.default.error('Failed to send password reset email', { email, error: error.message });
    ...
    async sendWelcomeEmail(email, name) {
        try {
            ...
            logger_1.default.error('Failed to send welcome email', { email, error: error.message });
        }
    }
    async sendStaffRegistrationEmail(email, name, role) {
        try {
            await this.transporter.sendMail({
                from: process.env.EMAIL_FROM || 'LS Radio <noreply@lsradio.com>',
                to: email,
            ...
            logger_1.default.info(`Staff registration email sent to ${email}`);
        }
        catch (error) {
            logger_1.default.error('Failed to send staff registration email', { email, error: error.message });
    ...
    getStaffRegistrationEmailTemplate(name, role) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
